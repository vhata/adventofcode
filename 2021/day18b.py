...
# For temporarily overriding the default representation type of SFNums
GLOB_REPTYPE = None
...
class SFNum:
    ...
    def __repr__(self):
        if not GLOB_REPTYPE:
            ...
            reptype = GLOB_REPTYPE
        if reptype == REPR_BASIC:
            return f"[{self.left},{self.right}]"
        if reptype == REPR_DAD:
            return f"[({self.dadcount}){self.left},{self.right}]"
        if reptype == REPR_COL:
            return f"{NORM}[{BRIGHT}{self.left}{NORM},{BRIGHT}{self.right}{NORM}]"
        if reptype == REPR_DAD_COL:
            return f"{NORM}[{RED}({self.dadcount}){NORM}{BRIGHT}{self.left}{NORM},{BRIGHT}{self.right}{NORM}]"

    def __add__(self, o):
        # BIG HACK!
        # Adding two SFNums modifies the originals, because we reduce them
        # after adding, and the SFNum trees are stored by reference, so the
        # originals get modified in the reduction.
        # Implementing __copy__ or __deepcopy__ isn't easy without getting
        # frightful recursion errors, so we fake it by simply serializing
        # the SFNum to its basic representation (using the GLOB_REPTYPE
        # override in case the default is something fancy), and then
        # re-parse it from that string, creating an all-new SFNum
        global GLOB_REPTYPE
        old_GLOB_REPTYPE = GLOB_REPTYPE
        GLOB_REPTYPE = 0

        newself, _ = parseSF(str(self))
        newo, _ = parseSF(str(o))
        result = SFNum(newself, newo)
        result.left.dad = result.right.dad = result
        result.reduce()

        GLOB_REPTYPE = old_GLOB_REPTYPE
        return result
...
x = list(itertools.permutations(nums, 2))

maxmag = 0
for p in x:
    maxmag = max(maxmag, int(p[0] + p[1]))

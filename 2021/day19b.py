...
def unrotate(m, rotation, signs):
    unrotation = [rotation[1], rotation[2], rotation[0]]
    unsigns = [signs[2], signs[0], signs[1]]
    return rotategrid(m, rotation, signs)
...
for i in scanners:
    for j in scanners:
        max_dist = max(max_dist, i.location.manhattan_dist(j.location))
